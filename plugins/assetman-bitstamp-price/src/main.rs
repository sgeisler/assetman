...
fn main() {
    ...
    let info = PluginInfo {
        ...
        description: "Returns the BTC price in the currency pair (e.g. BTCUSD) given as argument".to_string(),
    };
    ...
    Deserializer::from_reader(&mut stdin)
        ...
        .map(
            |req: Result<Request, _>| -> Result<Answer, assetman_api::Error> {
                ...
                let biststamp_resp = get(&format!("https://www.bitstamp.net/api/v2/ticker/{}", req.arguments)).map_err(|e| {
                    assetman_api::Error {
                        code: 3,
                ...
                let price = biststamp_resp.json()
                    ...
                    .and_then(|json: Value| -> Result<f64, assetman_api::Error> {
                        json.get("bid").ok_or(assetman_api::Error {
                            code: 5,
                            description: format!("Invalid API response, no 'bid' field.")
                        })?.as_str().ok_or(assetman_api::Error {
                            code: 6,
                            description: format!("Invalid API response: 'bid' is not a string!")
                        })?.parse().map_err(|e| {
                            assetman_api::Error {
                                code: 7,
                                description: format!("Invalid API response: 'bid' is not a valid float string: {:?}", e)
                            }
                        })
                    })?;
